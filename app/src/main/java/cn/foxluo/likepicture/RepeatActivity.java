...
public class RepeatActivity extends AppCompatActivity {
    ArrayList<ArrayList<PhotoBean>> repeatPhotoGroups = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new PhotosGroupAdapter(repeatPhotoGroups, photoChecks, opens, RepeatActivity.this, new PhotosGroupAdapter.OnGroupPhotosClickListener() {
            @Override
            public void photoClick(int groupIndex, int photoIndex) {
                BGAPhotoPreviewActivity.IntentBuilder photoPreviewIntentBuilder = new BGAPhotoPreviewActivity.IntentBuilder(RepeatActivity.this)
                        .saveImgDir(null);
                if (repeatPhotoGroups.get(groupIndex).size() == 1) {
                    photoPreviewIntentBuilder.previewPhoto(repeatPhotoGroups.get(groupIndex).get(0).getPath());
                } else if (repeatPhotoGroups.get(groupIndex).size() > 1) {
                    ArrayList<String> showPhotoUrls = new ArrayList<>();
                    for (PhotoBean photoBean : repeatPhotoGroups.get(groupIndex)) {
        ...
        for (int i = 0; i < photoChecks.size(); i++) {
            for (int j = 0; j < photoChecks.get(i).size(); j++) {
                if (photoChecks.get(i).get(j)) {
                    size += repeatPhotoGroups.get(i).get(j).getSize();
                    num++;
                }
            }
        }
                            ...
                            try {
                                ...
                                for (int i = 0; i < datePhotos.size(); i++) {
                                    ...
                                    for (int j = i + 1; j < datePhotos.size(); j++) {
                                        try {
                                            if (distance(datePhotos.get(i).getHashCode(), datePhotos.get(j).getHashCode()) <= 5) {
                                    ...
                                    if (photoBeans.size() > 0) {
                                        repeatPhotoGroups.add(photoBeans);
                                    }
                                }
    ...
    Handler doneHandler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            if (msg.what == 1) {
                photoChecks.clear();
                opens.clear();
                Collections.sort(repeatPhotoGroups, (o1, o2) ->
                        {
                ...
                for (ArrayList<PhotoBean> photoBeans : repeatPhotoGroups) {
                ...
                Toast.makeText(RepeatActivity.this, "扫描完成,时间花费: " + (time / 1000) + "s", Toast.LENGTH_LONG).show();
                ((TextView) findViewById(R.id.textView)).setVisibility(View.VISIBLE);
                ((TextView) findViewById(R.id.textView)).setText("共" + repeatPhotoGroups.size() + "组重复图片,占用空间" + format(size));
